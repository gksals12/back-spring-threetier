...
public class MemberAPI {
    ...
    @ApiResponse(responseCode = "201", description = "이메일 중복 여부 확인")
    ...
    public ResponseEntity<ApiResponseDTO> register(MemberVO memberVO) {
        ...
        return ResponseEntity.status(HttpStatus.CREATED).body(ApiResponseDTO.of("회원가입완료"));
    }
    ...
    public ResponseEntity<ApiResponseDTO> login(MemberVO memberVO) {
        MemberResponseDTO currentMember  =  memberService.login(memberVO);
         return ResponseEntity.status(HttpStatus.OK).body(ApiResponseDTO.of("로그인 완료", currentMember));
    }

    @Operation(summary = "회원 정보 수정", description = "회원의 정보를 수정 할 수 있는 API")
    @ApiResponse(responseCode = "200", description = "회원 정보 수정 성공")
    @PutMapping("modify")
    public ResponseEntity<ApiResponseDTO> updateMember(MemberVO memberVO) {
    ...
    @Operation(summary = "회원 탈퇴", description = "회원탈퇴를 할 수 있는 API")
